...
def estimate_page_scale(comps: list[Component], shape: tuple[int, int]) -> tuple[float, float, int, int]:
    ...
    band = [
        c for c in cands
        if 0.65 * mode_h <= c.h <= 1.55 * mode_h and c.line_support >= 0.22
    ]
    if len(band) < 20:
...
def bbox_overlap_ratios(a: Component, b: Component) -> tuple[float, float]:
    ...
    return (
        xo / max(1, min(a.w, b.w)),
        yo / max(1, min(a.h, b.h)),
    )
...
def _cells_for_box(x1: float, y1: float, x2: float, y2: float, cell: float) -> Iterable[tuple[int, int]]:
    ...
    ix2, iy2 = int(math.floor(max(x1, x2 - 1e-6) / cell)), int(math.floor(max(y1, y2 - 1e-6) / cell))
...
def build_bbox_neighbor_graph(comps: list[Component], max_gap: float) -> dict[int, list[Edge]]:
    ...
    # Insert expanded coverage. A true bbox-gap <= max_gap pair must share at least
    # one queried cell; the exact relation is checked below before an edge exists.
    for c in comps:
...
def growth_edge_cost(
    ...
    if edge.dx == 0.0 or edge.dy == 0.0:
        geometric *= 0.62
    geometric *= 1.0 - 0.30 * min(axis_continuity, 1.0)

    artifact = 0.0
    if nxt.area < 0.35 * h_cap * w_stroke and max(nxt.w, nxt.h) < 1.2 * h_cap:
        artifact = 1.25

    # Strong row evidence is more dangerous than isolated glyph-like geometry.
...
def grow_seed_families(
    ...
    while heap:
        ...
        for edge in graph.get(label, ()):
            ...
            if new_cost <= growth_budget and (new_cost < old_cost - 1e-9 or (abs(new_cost - old_cost) <= 1e-9 and seed_index < old_owner)):
                dist[other] = new_cost
    ...
    for seed_index, members in groups.items():
        ...
        regions.append(Region(
            component_labels=sorted(c.label for c in members),
...
def _region_pair_evidence(
    ...
    bbox_y_overlap = _overlap_1d(ay1, ay2, bx1, bx2) / max(1, min(ay2 - ay1, by2 - by1))
    return {
        "min_support_gap": float(min_gap),
        "best_axis_overlap": float(best_axis_overlap),
        "close_cross_edges": int(close_edges),
        "cross_edge_count": len(cross),
        "area_ratio": float(area_ratio),
        "bbox_x_overlap": float(bbox_x_overlap),
        "bbox_y_overlap": float(bbox_y_overlap),
    }
...
def _should_group(ev: dict, h_cap: float) -> bool:
    ...
    if area_ratio <= 0.30 and gap <= 1.35 * h_cap and (axis >= 0.08 or close_edges >= 1):
        return True
...
def _merge_regions(a: Region, b: Region, by_label: dict[int, Component], evidence: dict) -> Region:
    ...
    return Region(
        ...
        max_seed_score=max(a.max_seed_score, b.max_seed_score),
        grouping_evidence=a.grouping_evidence + b.grouping_evidence + [evidence],
    )
        ...
        for i in range(len(current)):
            for j in range(i + 1, len(current)):
                ...
                if not _should_group(ev, h_cap):
                    continue
                score = ev["min_support_gap"] / max(h_cap, 1.0) - 0.10 * ev["close_cross_edges"]
...
def renderable_support(
    ...
    support = lut[labels]
    return support, {
        "core_labels": sorted(core),
        "reachable_labels": sorted(dist),
        "support_labels": sorted(keep),
        "rejected_text_labels": sorted(rejected_text),
    }
...
def closure_and_alpha(
    support: np.ndarray,
    w_stroke: float,
    closure_budget: float,
    closure_boundary_min: float,
    max_bridge_width: float,
    margin_scale: float,
    alpha_scale: float,
    ...
    for scale in (0.75, 1.25, 1.75, 2.5):
        ...
        if hole_px:
            ...
            near_radius = max(1, int(round(w_stroke)))
            support_near = cv2.dilate(
                support,
                cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2 * near_radius + 1, 2 * near_radius + 1)),
            )
        ...
        accepted = (
            hole_px > 0
            and closure_cost <= closure_budget
            and boundary_support >= closure_boundary_min
            and max_bridge <= max_bridge_width
        )
        trial = {
            "scale": scale,
            "r_close_px": r_close,
            "bridge_pixels": bridge_px,
            "hole_pixels": hole_px,
            "closure_cost": float(closure_cost),
            "boundary_support": float(boundary_support),
            "max_bridge_width_wstroke": float(max_bridge),
    ...
    alpha = np.where(
        hard > 0,
        255.0,
        np.clip(255.0 * (1.0 - dist_outside / r_alpha), 0.0, 255.0),
    ).astype(np.uint8)

    return hard, alpha, {
        "closure_accepted": chosen is not None,
        "chosen": chosen,
        "trials": trials,
...
def page_domain(gray: np.ndarray) -> tuple[str, dict]:
    stats = {
        ...
        "near_white_fraction": float((gray > 245).mean()),
    }
...
def process_page(path: Path, out_dir: Path, args: argparse.Namespace) -> dict:
    ...
    payload: dict = {
        "schema": "corpus-motuum-figure-structure-probe-v2",
        "source": str(path),
    ...
    for index, region in enumerate(assets, start=1):
        support, support_diag = renderable_support(
            labels,
            region,
            by_label,
            graph,
            ...
            support_text_penalty=args.support_text_penalty,
        )
        hard, alpha, closure_diag = closure_and_alpha(
            support,
            w_stroke,
            closure_budget=args.closure_budget,
            closure_boundary_min=args.closure_boundary_min,
        ...
        files = {
            "support": out_dir / f"{prefix}.support.png",
            "territory": out_dir / f"{prefix}.territory.png",
            "alpha": out_dir / f"{prefix}.alpha.png",
            "source": out_dir / f"{prefix}.source.png",
            "clean": out_dir / f"{prefix}.clean.png",
        }
        shas = {
            "support": checked_imwrite(files["support"], crop_support),
            "territory": checked_imwrite(files["territory"], crop_hard),
            "alpha": checked_imwrite(files["alpha"], crop_alpha),
            "source": checked_imwrite(files["source"], source_rgba),
            "clean": checked_imwrite(files["clean"], clean_rgba),
        }
        ...
        asset_records.append({
            ...
            "territory_pixels": int(np.count_nonzero(crop_hard)),
    ...
    page_files = {
        "foreground": out_dir / f"{stem}.foreground.png",
        "ink_response": out_dir / f"{stem}.ink-response.png",
        "asset_overlay": out_dir / f"{stem}.asset-overlay.jpg",
    }
    page_shas = {
        "foreground": checked_imwrite(page_files["foreground"], fg),
        "ink_response": checked_imwrite(page_files["ink_response"], ink),
        "asset_overlay": checked_imwrite(
            page_files["asset_overlay"],
            overlay,
    ...
    payload.update({
        ...
        "parameters": {
            "graph_gap_scale": args.graph_gap_scale,
            "text_penalty": args.text_penalty,
            "growth_budget": args.growth_budget,
            "support_budget": args.support_budget,
            "support_text_penalty": args.support_text_penalty,
        ...
        "page_files": {k: p.name for k, p in page_files.items()},
        "page_file_sha256": page_shas,
...
def main() -> None:
    ...
    index_path.write_text(
        json.dumps(
            {
                "schema": "corpus-motuum-figure-structure-index-v2",
                "pages": pages,
                "failures": failures,
            },
            ensure_ascii=False,
